...
"""
color_list = ['red', 'blue', 'green', 'black'] # Palette 2
"""#"""
...
def main():
    ...
    for i in path_list:
        with open(i, 'r') as file:
            raw = np.array([l.split('\t') for l in file], dtype = 'float')
        raw_x = raw[:,0]
        raw_y = raw[:,1]
        # Normalize
    ...
    def plot(x,y,num):
        # Set Labels
        ax.set_title('ESI-TOFMS Overlay')
        ax.set_xlabel('m/z')
        ax.set_ylabel('[%]')
    ...
    if save == True:
        # Set file name
        save_fig_name = str(strftime("%y%m%d_%H%M%S", localtime())) + '_ESI_overlay.png'
        # Save figure
