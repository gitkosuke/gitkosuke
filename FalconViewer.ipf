...
Menu "&Analysis"
submenu "Binding"
	"Auto",  /Q, MakeTrackPanel()
	"Manual",  /Q, MakeDwellAnalysisPanel()
end
	"Curvature",  /Q,  MakeCurvatureAnalysisPanel()
	"Statistics",  /Q,  MakeStatsPanel() 
	"Image Volume",  /Q,  MakeVolumePanel()
	"Image Slicer",  /Q,  MakeImageSlicerPanel()
	"Force Review", /Q, MakeForceReviewPanel()
	"HS Force Review", /Q, ForcePanel()
	"HS Force Map", /Q, DoForceMapPanel()
	"NanoMech Result Analysis", /Q, NanoMechResultPanel()
...
Menu "&Image Processing"
	...
	"DeStripes/F6",  /Q, MakeDeStripesPanel()
	"Filter/F7",  /Q, ImageProcessing(2)
	"Frequency Filter/F8",  /Q, MakeFFTFilterPanel()
	"Magnify/F9", /Q, MakeMagnifyPanel()
	"Averaging/F11", /Q, MakeAveragingPanel()
	"Resampling/F12", /Q,  MakeInterpolationPanel()
	"Image Contrast", /Q, CreateImageContrastGraph()
	"Colors", /Q, MakeColorPanel()
	//	"Movie/F9",  /Q,  ImageProcessing(2)
end
...
//		//MakeBRTrackingPanel()
//
//		//プロシージャファイル: "C:Program Files:WaveMetrics:Igor Pro Folder:AsylumResearch:Mad Temp:Menu.ipf"
...
Function CreateViewerVariables()
  ...
  Wave InLineForceP
	if(WaveExists(InLineForceP)==0)
		Make/N=4  InLineForceP
		InLineForceP[0]=270.75
		InLineForceP[1]=94.25
		InLineForceP[2]=705
		InLineForceP[3]=530.25
	endif
...
Wave KymoP
	...
	//////////////////////////////////////////////////////////////
	///////////////////High-Speed Force Review ///////////////////
	
	
	if(DataFolderExists("root:HSRNM"))
	
		KillDataFolder/Z root:HSRNM
	endif
     	
	NewDataFolder/O/S  root:HSRNM
	...
	variable/G TempCapFlag=0
	variable/G TempCap_Size=18
	variable/G TempCap_LPosi=0
	variable/G TempCap_TPosi=40
...
Function CreateImageProssVariables()
	...
	variable/G CheckHistFlatten1ch
	variable/G CheckHistFlattenFit
	...
	variable/G  ManualRBGPositionX
	variable/G  ManualRBGPositionY
	variable/G  ManualRBGAngleX
	variable/G  ManualRBGAngleY
	variable/G  ManualRBGFactorX
	variable/G  ManualRBGFactorY
   Variable/G  ManualRBGOrderX
	Variable/G  ManualRBGOrderY

	variable/G  CheckManualRBGALL
	variable/G  CheckManualRBGActive
	...
	CheckHistFlatten1ch=0	
	CheckHistFlattenFit=0
...
Function MakeViewPanel(mode)
	...
	if(mode==0) //Imaging modeから入った場合	
		...
		if(V_Flag==0) 
			...
			MoveWindow/W=MainViewPanel MainViewPanelParams[0], MainViewPanelParams[1], MainViewPanelParams[2], MainViewPanelParams[3]
		  ...
		  // if(TempFlag==0)	
			ValDisplay ShowTemperature,win=MainViewPanel,  pos={CurrentLeft+5,CurrentTop+50}, frame =0, size={135,15}, title="Temperatre: ", format="%.1f C",value= #"root:ViewVariables:Temperature", disable=1//, proc=Check2chShowProc			
			//else
			//ValDisplay ShowTemperature,win=MainViewPanel,  pos={CurrentLeft+5,CurrentTop+50}, title="Temperatre: ", variable=Temperature, disable=0//, proc=Check2chShowProc			
			...
			TitleBox ShowDataFolder,win=MainViewPanel,  pos={CurrentLeft,CurrentTop},size={200,100},frame=0, variable=textDisplayStringPath	
			...
			CurrentTop +=CtrlSpace
			ListBox ImageList,win=MainViewPanel, font="Arial",fsize=10,frame=2,mode=4,selWave=root:ViewVariables:ImageListBuddy
			...
			ValDisplay ShowFirstFrame,win=MainViewPanel, pos={CurrentLeft+150,CurrentTop+60},frame=0, size={60,10},title="From"
			ValDisplay ShowFirstFrame,win=MainViewPanel, value= #"root:ViewVariables:FirstFrame", valueBackColor=(61000,61000,61000 )			
			ValDisplay ShowLastFrame,win=MainViewPanel, pos={CurrentLeft+210,CurrentTop+60},frame=0, size={60,10},title="to"
			...
			CurrentTop -=(20+18)
			CurrentLeft+=130                     
			Button buttonAutoView,win=MainViewPanel, pos={CurrentLeft,CurrentTop},size={20,22},disable=0,proc=ButtonAutoView,title="》》"
			...
			CurrentTop = 20
			CurrentLeft +=145
			GroupBox groupTempCap,pos={ CurrentLeft,CurrentTop},size={155,295},title="Temperature", fstyle=1, disable=1
	
			CurrentTop +=20
			CurrentLeft +=15
			CheckBox CheckTempCaption, win=MainViewPanel, pos={CurrentLeft,CurrentTop}, title="Add", variable=TempCapFlag , proc=CheckAddTemp, disable=1		
	
			CurrentTop +=25
			SetVariable setvarTempCapSize,win=MainViewPanel, pos={CurrentLeft,CurrentTop},size={85,16},value=TempCap_Size, title="Size", proc=SetTempCapProc, disable=1
	
	
			CurrentTop +=25
			SetVariable setvarTempLPosi, win=MainViewPanel, pos={CurrentLeft,CurrentTop},size={130,24},title="Left", value=TempCap_LPosi, format="%.1f%", proc=SetTempCapProc, disable=1
			SetVariable setvarTempLPosi, win=MainViewPanel, limits={-10,50,1}
	
			CurrentTop +=25
			SetVariable setvarTempTPosi, win=MainViewPanel, pos={CurrentLeft,CurrentTop},size={130,24},title="Top", value=TempCap_TPosi, format="%.1f%", proc=SetTempCapProc, disable=1
			SetVariable setvarTempTPosi, win=MainViewPanel, limits={0,100,1}		
	...
	if(mode==1) //TabControlで変更した場合 0: View 1: Save		
		...
		if(TabMode==1)		
			MoveWindow/W=MainViewPanel MainSavePanelParams[0], MainSavePanelParams[1], MainSavePanelParams[2], MainSavePanelParams[3]			
		else		
			MoveWindow/W=MainViewPanel MainViewPanelParams[0], MainViewPanelParams[1], MainViewPanelParams[2], MainViewPanelParams[3]
		endif	     
		...
		ValDisplay ShowTemperature,win=MainViewPanel,disable=1
		...
		elseif(TabMode==1)		
			...
			if(v_flag==1)			
				DoWindow/K Image1ch
				ShowImage(1)				
				DoAutoSizeImageFalcon(MovieSize,1, 1)				
			endif
			...
			if(v_flag==1)			
				DoWindow/K Image2ch				
				ShowImage2ch(1)				
				DoAutoSizeImageFalcon(MovieSize,1, 2)
			endif
...
Function ButtonSelectFolder(ctrlName) : ButtonControl
	...
	ListBox ImageList,font="Arial",win=MainViewPanel, fsize=10,frame=2,mode=4,selWave=root:ViewVariables:ImageListBuddy
...
Function OpenSubFolder()
	...
	ListBox ImageList,font="Arial",fsize=10,frame=2,mode=4,selWave=root:ViewVariables:ImageListBuddy		
	ListBox ImageList,listWave=root:ViewVariables:ImageList, proc=ImageDisplayListFuncAction
...
Function ButtonAutoViewStopProc(ctrlName) : ButtonControl
	...
	NVAR AutoViewStatus=root:ViewVariables:AutoViewStatus     
	AutoViewStatus=0      
	Button buttonAutoViewStop,win=MainViewPanel, rename=ButtonAutoView, proc=ButtonAutoView,title="》》"
...
static Function ListBoxUpdate(eventCode,row, keys)
	...
	if(eventCode==4) //KeyBoard
		...
		ValDisplay ShowTemperature,win=MainViewPanel, disable=0
...
Function ShowImage(Process_flag)
	...
	Wave RawDisplayImage
	if(WaveExists(RawDisplayImage)==0)
		Make/N=(1,1) RawDisplayImage
	endif
	...
	NVAR CheckHistFlatten1ch=root:ImageProcessVariables:CheckHistFlatten1ch
	...
	if(CrossFilterFlag==0 || Process_flag ==0)
		...
		SetScale x, 0, XScanSize*1e-9, "m",DisplayImage
		SetScale y ,0, YScanSize*1e-9,"m", DisplayImage
		Duplicate/O DisplayImage, RawDisplayImage
	...
	if(CheckRemoveBG1ch==1)	
		...
		FastOP ROImask=1	
		ImageRemoveBackGround/R=ROImask/O/P=(SetRemoveOrder1ch)/W  DisplayImage
		
//		Duplicate/O DisplayImage BackGround
//		BackGround=0
//		
//		Wave W_BackgroundCoeff
//		
//		for(y=0; y<DimSize(DisplayImage,1); y+=1)
//				for(x=0; x<DimSize(DisplayImage,0); x+=1)
//				
//				
//		for(i=0; i<numpnts(W_BackgroundCoeff); i+=1)
//		
//		BackGround[x][y]  +=  W_BackgroundCoeff[i]*y^(i)
//		
//		endfor
//		endfor
//		 endfor
//		
	...
	NVAR CheckManualRBGActive=root:ImageProcessVariables:CheckManualRBGActive	
	NVAR CheckManualRBGAll=root:ImageProcessVariables:CheckManualRBGAll
 
   if(CheckManualRBGActive==1)
		NVAR imageindex
		
		Wave MRBGWave=root:ImageProcessVariables:MRBGWave
		//DisplayImage -= MRBGWave
	   
	   if(CheckManualRBGAll==0 && WaveExists($"root:ImageProcessVariables:SaveMRBGWave:MRBGWave_"+num2str(imageindex))==1)
		...
		if(CheckManualRBGAll==1 && WaveExists($"root:ImageProcessVariables:SaveMRBGWave:MRBGWaveAll")==1)  
		      
			wave MRBGWave=root:ImageProcessVariables:SaveMRBGWave:MRBGWaveAll
			DisplayImage -=MRBGWave                 
      endif
	...
	if(CheckRemoveLineBG1ch==1 || CheckMedianFlatten1ch==1 || CheckMedianDifFlatten1ch==1 ||  CheckMeanFlatten1ch==1 ||  CheckHistFlatten1ch==1)
		...
		elseif(CheckHistFlatten1ch==1)	
			FlattenHistogram(FlattenImage)
		endif       
	...
	NVAR /Z CheckDeStripesAll = root:ImageProcessVariables:DeStripes:CheckDeStripesAll
   if( NVAR_Exists(CheckDeStripesAll)) 
      
      if(CheckDeStripesAll == 1)
	...
	elseif(WindowCheck==0)
		...
		ModifyImage/W=Image1ch DisplayImage ctab= {*,*,myColors,0},ctabAutoscale=0,lookup= wCLU
	   
	endif
	
	NVAR ScaleMode=root:ViewVariables:ScaleMode
	variable zmax=WaveMAx(DisplayImage)
	variable zmin=WaveMin(DisplayImage)	
	...
	if(ScaleMode==0)
      	...
      	SetScale/I y,0,1,myColors
      	
		DoWindow Image1ch	      
		if(V_Flag==1)
			ModifyImage/W=Image1ch DisplayImage ctab= {zmin,zmax, myColors,0},ctabAutoscale=0,lookup= wCLU
		endif
		...
		if(CheckSwapChFlag==0)
		if(RangeMode==0)
			WaveStats/Q DisplayImage
			DisplayImage -=V_avg //v_min //V_avg
		elseif(RangeMode==1)
			WaveStats/Q DisplayImage
			DisplayImage -=V_max
		elseif(RangeMode==2)
		...
		zmin=WaveMAx(DisplayImage)
		zmax=WaveMin(DisplayImage)	
	
      	SetScale/I x,zmin,zmax,myColors	
      	SetScale/I y,0,1,myColors
      	
		ModifyImage/W=Image1ch DisplayImage ctab= {nzmin,nzmax,myColors,0},ctabAutoscale=0,lookup= wCLU
		DoUpDate 
	...
	if(v_flag ==1)
		...
		if(DimSize(StackedImage,0)<DimSize(TrackedImage,0)|| DimSize(StackedImage,1)<DimSize(TrackedImage,1))
		 return 0
		endif
	...
	if(v_flag ==1)
		...
		if(DimSize(StackedImage,0)<DimSize(ShiftImage,0)|| DimSize(StackedImage,1)<DimSize(ShiftImage,1))
		 return 0
		endif
	...
	DoWIndow KymographPanel	
	if(v_flag ==1)
	
	SetDataFolder root:analysis:Kymograph

	
	Duplicate/O DisplayImage KymographImage
	SetScale/P x 0,1,"", KymographImage
	SetScale/P y 0,1,"", KymographImage
	...
	Val=WaveMin(KymographImage)
    ImageRotate/Q/Z/A=(RotAngle)/E=(Val) KymographImage
   
    
    Wave M_RotatedImage=root:Analysis:Kymograph:M_RotatedImage
    Duplicate/O M_RotatedImage KymographImage
	...
	/// Force Mapping
	 ...
	 //SetFCProc("SetFCLayer",ImageIndex, num2str(ImageIndex), "Layer")
	
	/// Force Mapping
	DoWindow ForceMapPanel
	NVAR SearchCurves = root:HSRNM:SearchCurves	
	if(v_flag ==1 && SearchCurves==1)
	...
	//Variable UnderBarPos=searchBackwards(BaseName,"_")
	//if (UnderBarPos< 0 )
	//	UnderBarPos = strlen(BaseName)
	//endif
	
	
	//String FolderNumStr=BaseName[0,UnderBarPos+2]
	
	//Variable Suffix=str2num(BaseName[UnderBarPos+3,strlen(BaseName)])

	String dfm_Name = BaseName + ".dfm"
	
	GetFileFolderInfo/P=ViewPath/Q/Z dfm_Name //aa
	
	if(v_flag==0)
	
	SVAR path = root:HSRNM:path
...
//ShowLaser()
		...
		if(V_flag==1)
			...
			Wave TwoPImage=root:Analysis:TwoPAnalysis:TwoPImage
		  
			Duplicate/O DisplayImage TwoPImage
		   //
			//ShowMarkDA()
	...
	if(v_flag ==1)
	   ...
	   NVAR ProcessStep=root:Analysis:Track:ProcessStep
	   NVAR CheckShowPos=root:Analysis:Track:ProcessStep
	   NVAR CheckShowRadius=root:Analysis:Track:CheckShowRadius
	   NVAR CheckShowID=root:Analysis:Track:CheckShowID
	   
	   NVAR PreFilter=root:Analysis:Track:PreFilter
	
	   if(ProcessStep==1)
	   
	   MakeCurrentIntHistogram()
		...
		if(Prefilter==1)
				...
				LogFilter(DisplayImage)

			else
	
				DoGFilter(DisplayImage)
...
Function ShowImage2ch(Process_flag)
	...
	Wave DisplayImage2ch=root:ViewVariables:DisplayImage2ch
	Wave RawDisplayImage2ch=root:ViewVariables:RawDisplayImage2ch
	...
	if(Process_flag ==0) 
		Duplicate/O DisplayImage2ch, RawDisplayImage2ch
	elseif(Process_flag !=0)
		DisplayImage2ch=RawDisplayImage2ch
	endif	
	
	//WaveStats/Q DisplayImage2ch
	
	//DisplayImage2ch -= v_avg
	...
	if(WindowCheck==1)
		...
		///if(YPixel != OldYPixel)
			GetWindow Image1ch, wsize		   
		//	MoveWIndow/W=Image2ch height/3+10,10,height/3+height/3,height/3*YScanSize/XScanSize
			//MoveWIndow/W=Image2ch v_right+10,v_top,v_right+(v_right-v_left), v_bottom+10
          MoveWIndow/W=Image2ch V_right+10,V_top,v_right+10+(v_right-v_left),v_top+height/3*YScanSize/XScanSize
...
Function CheckCrossFilterProc(ctrlName,checked) : CheckBoxControl
	...
	if(checked==0)	
		...
		Duplicate/O RawDisplayImage DisplayImage
...
//	Variable Suffix=str2num(BaseName[UnderBarPos+3,strlen(BaseName)])
//
//	String dfm_Name=FolderNumStr+"_"+num2str(Suffix)+".dfm"


	String dfm_Name = BaseName + ".dfm"
	
	GetFileFolderInfo/P=ViewPath/Q/Z dfm_Name
	...
	///　In-Line Force Curves
	
	DoWindow/F InLineForcePanel
	if(v_flag ==1)
	
	InLineForceFileOpen(ImageFilePath, FileName)
	
	NVAR ForceExist=root:InLineForce:ForceExist
	NVAR CheckShowPoint=root:InLineForce:CheckShowPoint
	...
	if(CheckShowPoint==0)
	  ...
	  CheckShowPointProc("",1)
	  
	else  //CheckShowPoint
	 
	  CheckShowPointProc("",1)
	 
	endif  //CheckShowPoint
	
	
	DisplayForceCurve(1, 1)
...
Function MakeMovie()
	...
	variable realFrameRate=floor(1/(FrameTime*1e-3))
	
	PlayBack=Frames_per_Second/realFrameRate
	...
	if(SaveModeFlag==1 ) 
		...
		BaseName=FileNameOnly(FileName)+"_x"+num2str(PlayBack)
		...
		//MovieSavePath2ch=S_FileName[0,len-5]+"_2ch"+S_FileName[len-4,len]  
		...
		for(index=0;index<FrameNum;index+=1)
			...
			for (mloop=0; mloop<Frameloop;mloop +=1)
				...
				elseif(index ==1)
					...
					//						endif
					//						if(FileNameFlag==1)
					//							TextBox/w=Image2ch/K/N=textFileName  
					//						endif
					//					
					//					endif	
				...
				if(index==0 && mloop==0)
					...
					switch(MovieFormat)					
						case 1: 
							...
							//	if(Show2chFlag==1)
							//	NewMovie/CF=(CompFactor)/Z/O/F=(Frames_per_Second)  as MovieSavePath2ch
							//	endif											
							break
						case 2:
							NewMovie/CF=(CompFactor)/Z/O/F=(Frames_per_Second)/A  as MovieSavePath
							//  if(Show2chFlag==1)
							//	NewMovie/CF=(CompFactor)/Z/O/F=(Frames_per_Second)/A  as MovieSavePath2ch
							//	endif					   
							break				
					endswitch
...
Function ButtonSaveProc(ctrlName) : ButtonControl
	...
	if(SaveModeFlag==1 &&   (SaveBMPFlag==1 || SaveJPEGFlag==1 || SaveTIFFFlag==1 || SaveBCRFlag==1))  // Single File
		...
		if(SaveFloderFlag==0)
			...
			//Open/D/P=ViewPath/M="Select Folder and Type BaseName" refnum as BaseName
		else
		...
		for(index=0;index<FrameNum;index+=1)
			...
			NVAR TempCapFlag=root:ViewVariables:TempCapFlag
			if(TempCapFlag==1)
				AddTempCap()
			endif
...
Function SaveAsASD( SaveFloderFlag)
	...
	elseif(SaveModeFlag==2)
		startindex=FirstFrame
		endindex=LastFrame
	endif
	
	variable SaveFrameNum
	
	SaveFrameNum=endindex-startindex+1
	
	//variable TargetFramenum=FrameNum
	
	NVAR percentFinished =root:ViewVariables:percentFinished 
	...
	if(DataType2ch ==0)   	  //1ch data only
		...
		for(index=startindex; index<=Endindex;index+=1)
			...
			//////
			//Wave FFTImage=root:ViewVariables:FFT_Filter:DisplayImage_magShow
			
			//WaveStats/Q FFTImage
			
			//FFTImage*= 0.01 ///= (v_max-v_min)
			
			//Duplicate/O FFTImage DisplayImage
			//////	
		...
		percentFinished=(index-startindex)/(Endindex-startindex)*100
		DoUpDate
			...
			if(index==startindex)
				...
				FBinWrite/F=3 refnum, SaveFrameNum
				FBinWrite/F=3 refnum, SaveFrameNum
			...
			endif //indexがゼロのときだけファイルヘッダの書き込み
      ...
      Open refnum as SaveFileName
		...
		DoUpDate
		 ...
		 variable check
			for(Ynum=0; Ynum<YPixel; Ynum+=1)
				for(Xnum=0; Xnum<Xpixel;Xnum+=1)			   
					//data=(4096.0*(5-DisplayImage[Xnum][Ynum]))/PiezoConstZ/DriverGainZ/10.0
					
					data=(5.0-DisplayImage[Xnum][Ynum]/PiezoConstZ/DriverGainZ)*4096/10.0
					FBinWrite/F=2 refNum, data
		...
		DoUpDate
		    ...
		    CurrentNum=index-StartIndex
			FSetPos refnum, FileHeaderSize+(FrameHeaderSize+2*Xpixel*YPixel)*(SaveFrameNum+CurrentNum)
...
Function PopScaleModeProc(ctrlName,popNum,popStr) : PopupMenuControl
	...
	Wave wCLU=root:ViewVariables:wCLU
	...
	variable Wzmax, Wzmin
	variable i, j
	
	wave DisplayImage=root:ViewVariables:DisplayImage
	wave myColors=root:ViewVariables:myColors
   
   	Wzmax=WaveMAx(DisplayImage)
	Wzmin=WaveMin(DisplayImage) 
	...
	if(ScaleMode==0)
		...
		SetScale/I x,Wzmin,Wzmax,myColors
		ModifyImage/W=Image1ch DisplayImage ctab= {*,*,myColors,0},ctabAutoscale=0,lookup= wCLU
	...
	elseif(ScaleMode==1)
		...
		NVAR zmin=root:Packages:WMImProcess:ImageRange:zmin	
		NVAR zmax=root:Packages:WMImProcess:ImageRange:zmax	
		       
		zmin=DimOffset(hist,0)
		zmax=DimOffset(hist,0)+DimDelta(hist,0)*numpnts(hist)
		
		Wave zmaxDrag=root:Packages:WMImProcess:ImageRange:zmaxDrag
		Wave zminDrag=root:Packages:WMImProcess:ImageRange:zminDrag
		
		ModifyGraph/W=WMImageRangeGraph offset(zmaxDrag)={zmax,0}
		ModifyGraph/W=WMImageRangeGraph offset(zminDrag)={zmin,0}
...
Function PanelSetting()
	...
	Wave VolumeP=root:VolumeP
	Wave InLineForceP=root:InLineForceP
	Wave TrackP=root:TrackP
	Wave KymoP=root:KymoP
	Wave DeStripesP=root:DeStripesP
	Wave ForceMapP=root:ForceMapP
	...
	if(v_flag==1)
		...
		GetWindow  InLineForcePanel ,wsize
		InLineForceP[0]=v_left
		InLineForceP[1]=v_top
		InLineForceP[2]=v_right
		InLineForceP[3]=v_bottom
	endif
	...
	if(v_flag==1)
		...
		GetWindow  KymographPanel,wsize
		KymoP[0]=v_left
		KymoP[1]=v_top
		KymoP[2]=v_right
		KymoP[3]=v_bottom
	...
	if(v_flag==1)
		...
		GetWindow  DeStripesPanel,wsize
		DeStripesP[0]=v_left
		DeStripesP[1]=v_top
		DeStripesP[2]=v_right
		DeStripesP[3]=v_bottom
	endif
	
	DoWindow  ForceMapPanel
	if(v_flag==1)
	
		if(WaveExists(ForceMapP)==0)
			Make/N=4 ForceMapsP
		endif
	     
		GetWindow  ForceMapPanel,wsize
		ForceMapP[0]=v_left
		ForceMapP[1]=v_top
		ForceMapP[2]=v_right
		ForceMapP[3]=v_bottom
	endif
	...
	SaveWaveName+=";VolumeP"
	SaveWaveName+=";InLineForceP"
	SaveWaveName+=";TrackP"
	SaveWaveName+=";KymoP"
	SaveWaveName+=";DeStripesP"
	SaveWaveName+=";ForceMapP"
...
Function SetTempCapProc(ctrlName,varNum,varStr,varName) : SetVariableControl
	String ctrlName
	Variable varNum
	String varStr
	String varName
...
Function CheckAddTemp(ctrlName,checked) : CheckBoxControl
	String ctrlName
	Variable checked
	
	NVAR TempFlag=root:ViewVariables:TempFlag
	NVAR TempCapFlag=root:ViewVariables:TempCapFlag
	...
	if(TempCapFlag==1)
		...
		DoWindow Image1ch
		if(V_Flag==1)
			TextBox/w=Image1ch/K/N=textTemp
		endif
		
		DoWindow Image2ch
		if(V_Flag==1)
			TextBox/w=Image2ch/K/N=textTemp
		endif
...
Function AddTempCap()
	...
	variable CapTemp =ceil(Temperature*10)/10
		
	//	Temperature =CapTemp/10
	
	  String Temp="\f"
	
	//Temp +="\Z"+num2str(TempCap_Size)
	...
	DoWindow Image1ch
	if(V_Flag==1)
		TextBox /C/B=1/N=textTemp/F=0/A=LT/G=(r,g,b)/M/X=(TempCap_LPosi)/Y= (TempCap_TPosi)  Temp
	endif

	DoWindow Image2ch
	if(V_Flag==1)
		TextBox/w=Image2ch/C/B=1/N=textTemp/F=0/A=LT/G=(r,g,b)/M/X=(TempCap_LPosi)/Y=( TempCap_TPosi)  Temp
...
//SetDrawLayer/W=Image1ch/K USerFront
//if(index<=3)		
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.540991835450332,0.434900730498462,0.470737913486005,0.321637426900585
//SetDrawEnv/w=Image1ch linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.645317535195879,0.344257455644661,0.575063613231552,0.230994152046784
//
//elseif(index==4)
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
...
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.610687022900763,0.461988304093567,0.669211195928753,0.576023391812866
//
//elseif(index==7)
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
...
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.714019825272215,0.127883186638813,0.776081424936387,0.248538011695906
//
...
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.452926208651399,0.163742690058479,0.343511450381679,0.190058479532164
//
//
//elseif(index==13)
//
...
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.88295165394402,0.736842105263157,0.7735368956743,0.763157894736842
//
...
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.641221374045801,0.365497076023391,0.534351145038168,0.318713450292398
//
//elseif(index >=21 && index <=23)
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
//
...
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
//
//elseif(index >=25 && index <=26)
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine/w=Image1ch 0.188295165394402,0.616959064327485,0.246819338422392,0.730994152046784 
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
//DrawLine 0.569974554707379,0.789473684210526,0.628498727735369,0.903508771929825
//
//elseif(index >=27)
//
//SetDrawEnv linefgc= (65535,65535,65535),linethick= 4.00,arrowfat= 1.50, arrowlen= 9.00, arrow= 2;
...
//           
	...
	if(V_Flag==1)
		...
		SetDrawLayer/W=Image2ch/K USerFront
		SetDrawLayer/W=Image2ch USerFront
	
		if(ScaleFlag==1)
			SetDrawEnv/W=Image2ch linethick=ScaleBar_Thickness, xcoord=prel, ycoord=prel, linefgc=(r,g,b)
			DrawLine/W=Image2ch ScaleBar_LPosi/100, ScaleBar_TPosi/100, ScaleBar_LPosi/100+ValScale/(XScanSize/ScaleFactor),ScaleBar_TPosi/100
	

			SetDrawEnv/W=Image2ch fsize=ScaleCap_Font, textrgb=(r,g,b ), fstyle=1//, fname=Cap_Font
	
			DrawText/w=Image2ch  ScaleBar_LPosi/100+ScaleCap_LPosi/100, ScaleBar_TPosi/100+ScaleCap_TPosi/100,  StrScale //-ScaleCap_TPosi/100
		endif
...
Function DoAutoSizeImageFalcon(forceSize,flipVert, imagech )
	variable forceSize,flipVert, imagech
	...
	imagename= "DisplayImage" //imagename[0,p1-1]
	Wave w= DisplayImage //ImageNameToWaveRef("",imagename)
	
	elseif(imagech==2)
	
	imagename= "DisplayImage2ch"
	Wave w= DisplayImage2ch //ImageNameToWaveRef("",imagename)
...
Function ShowColorBar()
	...
	if(ScaleMode==0)
		...
		if(CheckSwapChFlag==0)    
			...
			SetScale/I y zmin, zmax,"V", ColorBar
   
		endif
		
		//SetScale/I y, 0, 1.5, myColors
		ModifyImage/W=ColorBarPanel ColorBar ctab= {*,*,myColors,0},lookup= wCLU//	ModifyImage/W=Image1ch DisplayImage, cindex=MyColors
	...
	for(i=0;i<100; i+=1)
			...
			if(CheckSwapChFlag==0)  
				...
				SetScale/I y nzmin, nzmax,"V", ColorBar
   
			endif
			
			ModifyImage/W=ColorBarPanel ColorBar ctab= {*,*,myColors,0},ctabAutoscale=0,lookup= wCLU//	ModifyImage/W=Image1ch DisplayImage, cindex=MyColors
			...
			variable satV=round(100*(nzmax-nzmin)/(zmax-zmin))
			variable terminate
			...
			for(i=0;i<=terminate; i+=1)
			
				ColorBar[][i] = i/satV*100
...
//	PopupMenu popupResamplingFunc,win=InterpolationPanel, mode=1,title="Function",pos={35,100},popvalue="Bilinear",value= #"\"nm;bilinear;cubic;spline;sinc\"",proc=PopResampleFucProc, disable =1
	...
	SetVariable setvarSplineDeg, win=InterpolationPanel, pos={35,100},size={130,14}, limits={2,5,1}, value= $"root:ViewVariables:SplineDeg", title="Spline Degree", disable =0
...
Function ButtonInterporationProc(ctrlName) : ButtonControl
   ...
   for(frame=0;frame<FrameNum;frame+=1)
		...
		Make/O/N=(Dimsize(InterPImage,0), Dimsize(InterPImage,1), FrameNum) StackedInetrPImage
		...
		StackedInetrPImage[][][frame]=InterPImage[p][q]
   
      endfor
   
      tu_SaveResampling1ch(StackedInetrPImage)
   ...
   for(frame=0;frame<FrameNum;frame+=1)
			...
			if(InterPMethod ==1)
			       ...
			       ImageInterpolate/F={(SamplingCoef*XPixel-1)/(XPixel-1), (SamplingCoef*YPixel-1)/(YPixel-1)}/DEST=InterPImage1ch Bilinear DisplayImage
			        ImageInterpolate/F={(SamplingCoef*XPixel-1)/(XPixel-1), (SamplingCoef*YPixel-1)/(YPixel-1)}/DEST=InterPImage2ch Bilinear DisplayImage2ch
			     
			 
			elseif(InterPMethod ==2)
			
				   ImageInterpolate/F={(SamplingCoef*XPixel-1)/(XPixel-1), (SamplingCoef*YPixel-1)/(YPixel-1)}/DEST=InterPImage1ch/D=(SplineDeg)  Spline DisplayImage
				   ImageInterpolate/F={(SamplingCoef*XPixel-1)/(XPixel-1), (SamplingCoef*YPixel-1)/(YPixel-1)}/DEST=InterPImage2ch/D=(SplineDeg)  Spline DisplayImage2ch
				 
		 endif
		 
		if(frame==0)
		
		Make/O/N=(Dimsize(InterPImage1ch,0), Dimsize(InterPImage1ch,1), FrameNum) StackedInetrPImage1ch
		Make/O/N=(Dimsize(InterPImage1ch,0), Dimsize(InterPImage1ch,1), FrameNum) StackedInetrPImage2ch
		
		endif
		
		RS_X=Dimsize(InterPImage1ch,0)
		RS_Y=Dimsize(InterPImage1ch,1)
		
		StackedInetrPImage1ch[][][frame]=InterPImage1ch[p][q]
		StackedInetrPImage2ch[][][frame]=InterPImage2ch[p][q]
   
      endfor
   
      tu_SaveResampling2ch(StackedInetrPImage1ch, StackedInetrPImage2ch)
...
string SavedDataFolder = GetDataFolder(1)
	...
	if( LaserFlag==0)
		...
		SetDrawEnv/W=Image1ch  xcoord=prel, ycoord=prel, fillfgc= (32792,65535,1),  linethick= 0.00
		DrawRect/W=Image1ch 75/100, 90/100, 98/100,99/100
	else
		SetDrawLayer/W=Image1ch/K USerFront
		SetDrawLayer/W=Image1ch USerFront
		
		SetDrawEnv/W=Image1ch  xcoord=prel, ycoord=prel,  fillpat=4 , fillfgc= (39321,1,31457),  linethick= 0.00
		DrawRect/W=Image1ch 75/100, 90/100, 98/100,99/100
	endif
End
...
Function ShowMarkonForMovie(index)
   ...
   SetDataFolder root:Analysis:'20170615_020010C2':
	Wave PosX1=X_G
	Wave PosY1=Y_G
	Wave PosX2=X_B
	Wave PosY2=Y_B
	
//	Duplicate/O PositionX_1 PosX1
//	Duplicate/O PositionX_2 PosX2
//	Duplicate/O PositionY_1 PosY1
//	Duplicate/O PositionY_2 PosY2
//	
//	PosX1 *=Dimdelta(DisplayImage,0)
//	PosX2 *=Dimdelta(DisplayImage,0)
//	
//	PosY1 *=Dimdelta(DisplayImage,1)
//	PosY2 *=Dimdelta(DisplayImage,1)
	...
	if(index==0)
		...
		ModifyGraph /W=Image1ch rgb(MarkA_Y)=(0,65535,0)
       
		ModifyGraph/W=Image1ch mode(MarkB_Y)=3,marker(MarkB_Y)=19,msize(MarkB_Y)=5,mrkThick(MarkB_Y)=1;
		ModifyGraph /W=Image1ch rgb(MarkB_Y)=(0,65535,65535)
